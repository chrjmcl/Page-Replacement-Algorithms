	...
	public static void handleException(ExceptionType pException) {
		...
		case PageFaultException:
			...
			if (ppn < 0) {
				...
				/* Project 2 FIFO Implementation */
				if(!FIFO.queue.contains(ppn)) {
					FIFO.queue.addLast(ppn);
				}
				
				/* Project 2 LRU Implementation */
				if(!LRU.queue.contains(ppn)) {
				LRU.queue.addLast(ppn);
				}
			}
