...
class TimerInterruptHandler implements VoidFunctionPtr {
	...
	public void call(Object pDummy) {
		...
		if(counter % 5 == 0) {
			counter++;
			/* Project 2 NRU Implementation */
			/* !COMMENT OUT IF USING NFU! */
			//NRU.update();
		}
		else { 
			counter++; 
			/* Project 2 NFU Implementation */
			/* !COMMENT OUT IF USING NRU! */
			NFU.update();
		}
...
public abstract class JNachos {
	...
	/** The page replacement used in virtual memory. */
	private static PageReplacementAlgorithm mPageReplacementAlgorithm;
	
	
	/** A map from page frame to the owning process's PID. */
	public static int[] mPageFrameToPid;
	...
	/*** project 2 ***/
	/* Make a swap space, using JavaFileSystem.java, the swap space is a file */
	public static OpenFile mSwapSpace;

	public static Map<String, AddrSpace> mSharedLibrarySpace;

	/* project 2 */
	/*
	 * swap space counter to keep track of the current index of the swap space
	 */
	public static int swapspace_counter;
	...
	public static void initialize(String args[]) {
		...
		if (mFileSystem.create("swapspace", 0) == true) {
			mSwapSpace = mFileSystem.open("swapspace");
		}

		mSharedLibrarySpace = new HashMap<String, AddrSpace>();
	...
	/**
	 * 
	 * @return the page replacement algorithm to use.
	 */
	public static PageReplacementAlgorithm getPageReplacementAlgorithm() {
		return mPageReplacementAlgorithm;
	}
	
	public static int[] getPageFrameMap() {
		return mPageFrameToPid;
	}
}
